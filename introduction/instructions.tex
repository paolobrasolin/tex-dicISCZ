...
\blspace
  \dicEntry \dicTermDemo{vor}<1> \dicPos{n} \dicFlx{(-s, -)}
  \dicEntry \dicTermDemo{vor}<2> \dicPos{pron} \dicFlx{pers gen pl}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{ærslafenginn}[ærsla··fenginn], \dicTermDemo*{ærslafullur} \dicPos{adj}
\blspace

nebo b) dublety (dvojtvary).

\blspace
  \dicEntry \dicTermDemo{ævintýri}[ævin··týri], \dicTermDemo*{ævintýr} \dicPos{n}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{ærslafullur}[ærsla··|fullur] \dicIPA{{a}{i}{r̥}{s}{d̥}{l}{a}{f}{ʏ}{d̥}{l}{ʏ}{r̥}} \dicPos{adj} \dicFlx{(comp -fyllri, sup -fyllstur)} \dicLink{ærslafenginn}
  \dicEntry \dicTermDemo{ævintýr}[ævin··týr] \dicIPA{{a}{i}{ː}{v}{ɪ}{n}{tʰ}{i}{r̥}} \dicPos{n} \dicFlx{(-s, -)} \dicLink{ævintýri}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{öttum} \dicPos{v} \dicFlx{ind pf pl 1 pers} \dicLink{etja}
\blspace
...
\blspace
\dicEntry \dicTermDemo{ráðhús}[ráð··hús]
nebo o~dělení mezi kořenem a~předponou / příponou.
\dicEntry \dicTermDemo{þjónusta}[þjón··usta]
\blspace
...
\blspace
\dicEntry \dicTermDemo{viðbótarlífeyrissparnaður}[við·bótar··líf·eyris·sparn·aður]
\blspace
\item V~hesle s~předponou je označena jedna nebo i~více předpon (každá zvlášť).
\blspace
\dicEntry \dicTermDemo{óaðfinnanlegur}[ó··að·finnan·legur]
\blspace
\item V~hesle s~příponou je označena jedna nebo i~více přípon (každá zvlášť).
\blspace
\dicEntry \dicTermDemo{léttúðugur}[létt·úð··ugur]
\blspace
...
\blspace
\dicEntry \dicTermDemo{flutningamiðlari}[flutninga··miðl·ari]
\end{enumerate}
...
\blspace
  \dicEntry \dicTermDemo{koma}<2> \dicsymFrequent
\blspace
...
\blspace
  \dicEntry \dicTermDemo{öldurót}[öldu··rót] \dicIPA{{\ae}{l}{d̥}{ʏ}{r}{ou}{d̥}}
\blspace

Znak '{ː}' označuje dlouhou samohlásku nebo souhlásku.

\blspace
  \dicEntry \dicTermDemo{ör}<2> \dicIPA{{\ae}{ː}{r̥}}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{af} \dicPos{prep / adv}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{hestur} \dicPos{m}
  \dicEntry \dicTermDemo{Hanoi} \dicPos{subs}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{fallegur} \dicPos{adj}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{yngri} \dicPos{adj} \dicFlx{comp m} \dicLink{ungur}
\blspace

nebo jako svébytná hesla s~význami.

\blspace
  \dicEntry \dicTermDemo{fremri} \dicPos{adj} \dicFlx{comp (sup fremstur)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{þessi} \dicPos{pron} \dicFlx{dem}
\blspace

U~zájmen přivlastňovacích je v~3. osobě označen rod.

\blspace
  \dicEntry \dicTermDemo{hans} \dicPos{pron} \dicFlx{poss m}
\blspace

U~zájmena osobních je označen pád a~číslo.

\blspace
  \dicEntry \dicTermDemo{mig} \dicPos{pron} \dicFlx{pers acc sg}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{tuttugu}[tut··tugu] \dicPos{num}
  \dicEntry \dicTermDemo{tveir} \dicPos{num} \dicFlx{pl m}
  \dicEntry \dicTermDemo{fyrsti} \dicPos{num} \dicFlx{ord}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{klóra} \dicPos{v} \dicFlx{acc / dat}
\blspace

V~případě, že se sloveso pojí s~více pády je použito znaménko plus.

\blspace
  \dicEntry \dicTermDemo{gefa} \dicPos{v} \dicFlx{dat + acc}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{þar} \dicPos{adv}
  \dicEntry \dicTermDemo{ofar} \dicPos{adv} \dicFlx{comp (pos uppi, sup efst)}
  \dicEntry \dicTermDemo{síðast} \dicPos{adv} \dicFlx{sup (comp síðar)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{af} \dicPos{prep / adv} \dicFlx{dat}
\blspace

V~případě, že se předložky pojí s~různými pády je použit znak \textbf{'/'}.

\blspace
  \dicEntry \dicTermDemo{fyrir} \dicPos{prep / adv} \dicFlx{acc / dat}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{hvalur}[hval|ur] \dicPos{m} \dicFlx{(-s, -ir)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{afarkostir}[afar··kostir] \dicPos{m} \dicFlx{pl}
\blspace

Pokud je použita zkratka \textbf{indecl}, znamená to, že podstatné jméno je nesklonné.

\blspace
  \dicEntry \dicTermDemo{október} \dicPos{m} \dicFlx{indecl}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{hýr} \dicPos{adj} \dicFlx{(f -)}
\blspace

Dále jsou uvedeny tvary ženského rodu, ve~kterých dochází k~přehlásce:

\blspace
  \dicEntry \dicTermDemo{reglusamur}[reglu··|samur] \dicPos{adj} \dicFlx{(f -söm)}
\blspace

Pokud je přídavené jméno nesklonné, je uvedena zkratka \textbf{indecl}.

\blspace
  \dicEntry \dicTermDemo{tvímála}[tví··mála] \dicPos{adj} \dicFlx{indecl}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{þunnur} \dicPos{adj} \dicFlx{(comp þynnri, sup þynnstur)}
\blspace

V~případě, že je uveden pouze jeden ze dvou zbývajících stupňů, znamená to, že heslo se stupňuje pouze v~uvedených stupních.

\blspace
  \dicEntry \dicTermDemo{syðri} \dicPos{adj} \dicFlx{comp (sup syðstur)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{ætla}[ætl|a] \dicPos{v} \dicFlx{(-aði)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{kenna}[kenn|a] \dicPos{v} \dicFlx{(-di, -t)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{grípa} \dicPos{v} \dicFlx{(gríp, greip, gripum, gripi, gripið)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{krossbregða}[kross··|bregða] \dicPos{v} \dicFlx{(-bregður, -brá, -brygði, -brugðið)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{batna}[batn|a] \dicPos{v} \dicFlx{(-aði)} \dicTermDemo{e-m batnar} \dicFlx{impers} (kdo) se uzdravuje, (kdo) se zotavuje, (komu) je lépe
\blspace

Zkratka \textbf{refl} označuje mediopasivní tvary slovesa.

\blspace
  \dicEntry \dicTermDemo{að··lag|a} \dicPos{v} \dicFlx{(-aði)} \dicIdiom{aðlagast}{aðlagast e-u} \dicFlx{refl} {přizpůsobit se (čemu)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{vel}<2> \dicPos{adv} \dicFlx{(comp betur, sup best)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{síst} \dicPos{adv} \dicFlx{sup (pos varla, comp síður)}
\blspace

Pokud některý ze stupňů uveden není, znamená to, že pro dané slovo neexistuje.

\blspace
  \dicEntry \dicTermDemo{síðar} \dicPos{adv} \dicFlx{comp (sup síðast)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{ap|i} \dicPos{m}[1] \dicFlx{(-a, -ar)}[a5]
\blspace
...
\blspace
  \dicEntry \dicTermDemo{kis|a} \dicPos{f}[1] \dicFlx{(-u, -ur)}[a1]
\blspace
...
\blspace
  \dicEntry \dicTermDemo{landa··fræð|i} \dicPos{f} \dicFlx{(-i)} \textCS{zeměpis, geografie}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{að··stoð} \dicPos{f} \dicFlx{(-ar)} \textCS{(vý)pomoc, asistence}
\blspace

V~druhém případě se v~závorce nachází upřesňující informace, která zabraňuje dvojznačnosti.

\blspace
  \dicEntry \dicTermDemo{fíkj|a} \dicPos{f} \dicFlx{(-u, -ur)} \textCS{fík} {\footnotesize {\textCS{(plod)}}}
\blspace

V~mnoha případech je závorka v~české části použita pro příklad užití českého slova a~tím vymezení jeho významu.

\blspace
  \dicEntry \dicTermDemo{að·gengi··legur} \dicPos{adj} {přístupný, dostupný (informace ap.)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{aðal··|gata} \dicPos{f} \dicFlx{(-götu, -götur)} \textCS{hlavní třída / ulice}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{til··ber|i} \dicPos{m} \dicFlx{(-a, -ar)} {\footnotesize{pov.}} {\footnotesize{stvoření, které dojí krávy a~ovce hospodářů}}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{lær|a} \dicPos{v}[(-ði, -t)] \textbf{læra e-ð utanbókar} {učit se (co) zpaměti, memorovat (co)}
\blspace

Závorky jsou použity na tu část slovního spojení, kterou je možné vypustit.

\blspace
  \dicEntry \dicTermDemo{ár}<1> \dicPos{f} \dicFlx{(-ar, -ar)} \textbf{taka (of) djúpt í árinni} {\footnotesize{přen.}} {přehnat (co v~tvrzení)}
\blspace
...
\blspace
\dicEntry \dicTermDemo{bót} \dicPos{f} \dicFlx{(-ar, bætur)} \textbf{bætur} \dicFlx{pl} {dávky, přídavky, příspěvky}
Dále do této skupiny řadíme tvar slovesa v~středním slovesném rodě (zkratka \textbf{refl}).
\dicEntry \dicTermDemo{finna} \dicPos{v} \dicFlx{(finn, fann, fundum, fyndi, fundið)} \dicIdiom{finnast}{finnast} \dicFlx{refl} {být nalezen, najít se, nalézt se, nacházet se}
\blspace
...
\blspace
\dicEntry \dicTermDemo{koma}<2> \dicPos{v} \dicFlx{(kem, kom, komum, kæmi, komið)} \dicIdiom{koma + að}{koma að e-m} {přiblížit se ke (komu), přijít ke (komu)}
\blspace
...
\blspace
\dicEntry \dicTermDemo{kast} \dicPos{n} \dicFlx{(-s, köst)} \dicIdiom{kast}{þegar til kastanna kemur} {\footnotesize{přen.}} {když jde do tuhého}
\blspace
...
\blspace
\dicEntry \dicTermDemo{æfing} \dicPos{f} \dicFlx{(-ar, -ar)} \dicProverb{Æfing skapar meistara.} {\footnotesize{přís.}} \textCS{Cvičení dělá mistra.}
\end{enumerate}
...
\blspace
  \dicEntry \dicTermDemo{land} \dicPos{n} \dicFlx{(-s, lönd)} {\textbf{1.}} \dicSynonym{þurrlendi} {souš, pevnina, země} {\textbf{2.}} \dicSynonym{árbakki} {břeh} {\textbf{3.}} \dicSynonym{ríki} {země, stát, vlast} {\textbf{4.}} \dicSynonym{landareign} {země, půda, pozemek}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{drag} {\dicPos{n} \dicFlx{(-s, drög)}{mokřina, podmoklý terén;} \textbf{drög} {\footnotesize{pl}} {\textbf{a.}} \dicSynonym{uppsprettur} {prameny (řeky ap.);} {\textbf{b.}}\dicSynonym{undirbúningur} {náčtr, návrh.}}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{tím|i} \dicPos{m} \dicFlx{(-a, -ar)} {\textbf{1.}} \dicSynonym{tíð} {čas, doba}; \textbf{í þann tíma} \dicFlx{adv} \dicSynonym{þá}<2> {pak} {\textbf{2.}} \dicSynonym{rétt stund} {(správný) čas / moment}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{finna} \dicPos{v} \dicFlx{(finn, fann, fundum, fyndi, fundið)} {\textbf{3.}}\dicSynonym{skynja} {cítit, vnímat} \dicIdiom{finna + að}{finna að e-u} {kritizovat (co), nacházet chyby na (čem)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{koma}<2> \dicPos{v} \dicFlx{(kem, kom, komum, kæmi, komið)} \dicIdiom{koma + út}{koma út} {vyjít (kniha ap.), objevit se (na trhu ap.)}
\blspace

Poté následují slovní spojení v~slovesném středním rodě řazená opět podle předložky / příslovce / zájmena, se kterou se pojí:

\blspace
  \dicEntry \dicTermDemo{koma}<2> \dicPos{v} \dicFlx{(kem, kom, komum, kæmi, komið)} \dicIdiom{komast}{komast} \dicFlx{refl} {dostat se, dostávat se}
\blspace

Poté následují ustálená slovní spojení, která nebylo možné zařadit do předchozích položek:

\blspace
  \dicEntry \dicTermDemo{koma}<2> \dicPos{v} \dicFlx{(kem, kom, komum, kæmi, komið)} \dicIdiom{koma}{(komdu) sæll (og blessaður)} {ahoj, nazdar, dobrý den}
\blspace

Nakonec jsou uvedena přísloví a~rčení označená symbolem \dicsymProverb:

\blspace
  \dicEntry \dicTermDemo{eik} \dicPos{n} \dicFlx{(-ar / -ur, -ur)} \dicProverb{Ekki / eigi fellur eik við fyrsta högg.} {\footnotesize{přís.}} {Jednou ranou dub nespadne}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{staða} \dicPos{f} \dicFlx{(stöðu, stöður)} {\textbf{1.}} \dicSynonym{ástand} {situace, stav, poměry;} {\textbf{2.}} \dicSynonym{embætti} {post, (pracovní) místo / pozice (ve firmě ap.)}
\blspace

V~případě, že synonymum lze nalézt jako heslo ve slovníku, je použita šipka '\dicsymSee'.

\blspace
  \dicEntry \dicTermDemo{bygging} {\dicPos{f} \dicFlx{(-ar, -ar)} {\textbf{1.}} \dicSynonym{hús} {budova, stavba, stavení;} {\textbf{2.}}\textit{(það að byggja)} {(vý)stavba} }
\blspace
...
\blspace
  \dicEntry \dicTermDemo{kaldur} \dicPos{adj} \dicFlx{(f köld)} {1.} {studený, chladný} \textit{($\shortuparrow$ heitur)}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{berg} \dicPos{n} \dicFlx{(-s, -)} {\textbf{1.}} {\footnotesize{geol.}} {hornina, kámen;} {\textbf{2.}} \dicSynonym{klöpp}<1> {skalní stěna, skalisko}
\blspace

a~vymezují význam českého slova.

\blspace
  \dicEntry \dicTermDemo{mús} \dicPos{f} \dicFlx{(-ar, mýs)} {\textbf{1.}} {\footnotesize{zool.}} {myš} (\textit{l. Mus}); {\textbf{2.}} {\footnotesize{poč.}} {myš}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{gaur} \dicPos{m} \dicFlx{(-s, -ar)} {\textbf{1.}} {\footnotesize{hovor.}} {chlápek, maník, člověk}
\blspace
...
\blspace
  \dicEntry \dicTermDemo{leit|a} \dicPos{v} \dicFlx{(-aði)} \textbf{leita að e-u / e-m} {hledat (co / koho)} \dicExampleIS{leita að lyklunum} \dicExampleCS{hledat klíče}
\blspace
...
\blspace
\dicEntry \dicTermDemo{hett|a} \dicPos{f} \dicFlx{(-u, -ur)} \textbf{1.} {kapuce, kápě} \dicExampleIS{hettuúlpa} \dicExampleCS{bunda s~kapucí}
\blspace
\item Slovní spojení jako praktický příklad:
\blspace
\dicEntry \dicTermDemo{tím|i} \dicPos{m} \dicFlx{(-a, -ar)} \textbf{3.} \dicSynonym{klukkustund} {hodina} {\footnotesize{(šedesát minut)}} \dicExampleIS{tveggja tíma gangur} \dicExampleCS{dvouhodinový pochod}
\blspace
\item Věta jako praktický příklad:
\blspace
\dicEntry \dicTermDemo{mæt|a} \dicPos{v} \dicFlx{(-ti, -t)} \dicIdiom{mætast}{(ná saman)} \dicFlx{refl} {setkat se, potkat se} \dicExampleIS{Þau mættust á gangstéttinni.} \dicExampleCS{Potkali se na chodníku.}
\end{enumerate}
