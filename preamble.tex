...
\documentclass[ demo, 8pt, c5paper, twoside, openright]{extbook}
...
\ifdummyfigures
  \let\olddicFigure\dicFigure
  \renewcommand\dicFigure[3]{\olddicFigure{example-image-a}{#2}{#3}}
\fi
...
\def\thLast{}  % Last thumb of previous page
\def\thStack{} % Stack holding thumbs of current page
...
\newcommand\drawrectothumb[1]{\drawthumb{#1}{east}{\csname thLabel#1\endcsname}}
\newcommand\drawversothumb[1]{\drawthumb{#1}{west}{\csname thLabel#1\endcsname}}

\def\dorectothumbs{\expandafter\thGobbleWith\thStack<\drawrectothumb>}
\def\doversothumbs{\expandafter\thGobbleWith\thStack<\drawversothumb>}
...
\fancypagestyle{myheadings}{
  ...
  \fancyhead[LE,LO]{\termFamily\bfseries\rightmark}
  \fancyhead[RE,RO]{\termFamily\bfseries\leftmark}
}
...
\NewDocumentCommand\dicStyleIntTerm{m}{\textIS{{\termFamily\bfseries#1}}}
\NewDocumentCommand\dicStyleExtTerm{m}{\textIS{\textit{#1}}}

\NewDocumentCommand\dicTerm{smod<>} % \dicTerm*{term}[te·r|m]<2>
  {\refstepcounter{terms}\label{term:#2\IfValueT{#4}{:#4}}%
   \dicStyleIntTerm{\IfValueTF{#3}{\textAlt{#3}{#2}}{#2}}%
   \IfValueT{#4}{\smash{\textsuperscript{#4}}}}
...
\NewDocumentCommand\dicLink{smd<>}
  {\IfBooleanF{#1}{\dicsymSee~}%
   \hyperref[term:#2\IfValueT{#3}{:#3}]{\dicStyleIntTerm{#2}}}
\NewDocumentCommand\dicSynonym{smd<>}
  {(\IfBooleanF{#1}{\dicsymSee~}\dicStyleExtTerm{#2})}
\NewDocumentCommand\dicAntonym{smd<>}
  {(\IfBooleanF{#1}{\dicsymCompare~}\dicStyleExtTerm{#2})}
...
\newcommand*{\dicLetter}[1]{%
  \FloatBarrier\newpage\phantomsection\pdfbookmark[1]{#1}{LETTER#1}%
...
\dicLetter{þ}\lipsum\lipsum\lipsum\lipsum\lipsum\lipsum
\dicLetter{Z}\lipsum\lipsum
\dicLetter{A}\lipsum
\dicLetter{B}
\dicLetter{T}
\dicLetter{S}\lipsum
